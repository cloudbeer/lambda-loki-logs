...
class ExtensionsAPIClient:
    def __init__(self):
        try:
            ...
            self.runtime_api_base_url = (
                f"http://{runtime_api_address}/2020-01-01/extension"
            )
    ...
    def register(self, agent_unique_name, registration_body):
        try:
            ...
            if resp.status != 200:
                print(
                    f"/register request to ExtensionsAPIClient failed. Status:  {resp.status}, Response: {resp.read()}"
                )
                # Fail the extension
                sys.exit(1)
            ...
            raise Exception(
                f"Failed to register to ExtensionsAPIClient: on {self.runtime_api_base_url}/register \
    ...
    def next(self, agent_id):
        try:
            ...
            if resp.status != 200:
                print(
                    f"/event/next request to ExtensionsAPIClient failed. Status: {resp.status}, Response: {resp.read()} "
                )
                # Fail the extension
                sys.exit(1)
            data = resp.read()
            ...
            raise Exception(
                f"Failed to get /event/next from ExtensionsAPIClient: {e}"
            ) from e
