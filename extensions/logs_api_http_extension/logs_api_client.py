...
class LogsAPIClient:
    ...
    def subscribe(self, agent_id, subscription_body):
        try:
            ...
            if resp.status == 202:
                print(
                    "extensions.logs_api_client: WARNING!!! Logs API is not supported! Is this extension running in a local sandbox?"
                )
            ...
            raise Exception(
                f"Failed to subscribe to Logs API on {self.logs_api_base_url} with id: {agent_id} \
